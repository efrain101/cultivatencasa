...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h2 style="color: #000000" align="center">Lista de efectos de las enfermedades</h2>
            </div>
            @if(Auth::user()->tipo_usuario==1)
            <div class="pull-right">
                <a class="button" href="{{url("efectos_enfermedades_c/create")}}" title="Registrar efecto de enfermedad"> <i class="fas fa-plus-circle"></i>
                </a>
            </div>
            @endif
        </div>
    ...
    <table class="table table-bordered table-responsive-lg" style="margin-top: 1em" >
        ...
        @foreach ($efectos_enfermedades_c as $efectos_enfermedades_c)
            <tr>
                <td>{{$efectos_enfermedades_c->efecto}}</td>
                @if(Auth::user()->tipo_usuario==1)
                    <td>
                        <table>
                            <tr>
                                <td>
                                    <form action="{{url("efectos_enfermedades_c", $efectos_enfermedades_c->id_efecto_enfermedad)}}" method="POST">
                                        @csrf
                                        @method("DELETE")
                                ...
                                <td>
                                    <a class="button" href="{{url("efectos_enfermedades_c",$efectos_enfermedades_c->id_efecto_enfermedad)."/edit"}}">
