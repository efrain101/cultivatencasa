...
@section('content')
    ...
    <div>
        <h1 id="tituloenfermedades" style="text-align: center" ><MARQUEE BEHAVIOR="ALTERNATE">Añadir enfermedad</MARQUEE></h1>
    </div>


    <div class="container">
        <form id="formuenfermedades" action="{{url("enfermedades")}}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="container">
                <div class="row" >
                    ...
                    <div class="col-4" style="font-family: 'Arial Black'">
                        ...
                        <select name="id_tipo_enfermedad" class="form-control @error('id_tipo_enfermedad') is-invalid @enderror" name="id_tipo_enfermedad"  style="margin-top: 2em">
                            @foreach($tipos_enfermedades_c as $tipos_enfermedades_c)
                                <option value="{{$tipos_enfermedades_c['id_tipo_enfermedad']}}">{{$tipos_enfermedades_c['tipo']}}</option>
                            @endforeach
                        </select>
