...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h2 style="color: #000000" align="center">Lista de prevenciones de las plagas</h2>
            </div>
            @if(Auth::user()->tipo_usuario==1)
            <div class="pull-right">
                <a class="button" href="{{url("prevencion_plagas_c/create")}}" title="Registrar prevención de plaga"> <i class="fas fa-plus-circle"></i>
                </a>
            </div>
            @endif
        </div>
    ...
    <table class="table table-bordered table-responsive-lg" style="margin-top: 1em" >
        ...
        @foreach ($prevencion_plagas_c as $prevencion_plagas_c)
            <tr>
                <td>{{$prevencion_plagas_c->prevencion}}</td>
                @if(Auth::user()->tipo_usuario==1)
                    <td>
                        <table>
                            <tr>
                                <td>
                                    <form action="{{url("prevencion_plagas_c", $prevencion_plagas_c->id_prevencion_plaga)}}" method="POST">
                                        @csrf
                                        @method("DELETE")
                                ...
                                <td>
                                    <a class="button" href="{{url("prevencion_plagas_c",$prevencion_plagas_c->id_prevencion_plaga)."/edit"}}">
