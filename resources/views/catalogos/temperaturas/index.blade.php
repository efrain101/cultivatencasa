...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h2 style="color: #000000" align="center">Lista de Temperaturas</h2>
            </div>
            @if(Auth::user()->tipo_usuario==1)
            <div class="pull-right">
                <a id="btnañadir" class="button" href="{{url("temperaturas/create")}}" title="Registrar temperatura"> <i class="fas fa-plus-circle"></i>
                </a>
            </div>
            @endif
        </div>
    ...
    <table class="table table-bordered table-responsive-lg">
        ...
        @foreach ($temperaturas as $temperatura)
            <tr>
                <td>{{$temperatura->valor_minimo}}</td>
                <td>{{$temperatura->valor_maximo}}</td>
                @if(Auth::user()->tipo_usuario==1)
                    <td>
                        <table>
                            <tr>
                                ...
                                <td>
                                    <a class="button" href="{{url("temperaturas",$temperatura->id_temperatura)."/edit"}}">
