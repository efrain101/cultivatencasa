...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h2 style="color: #ffffff" align="center">Efectos de las enfermedades</h2>
            </div>
            @if(Auth::user()->tipo_usuario==1)
            <div class="pull-right">
                <a id="btnañadir" class="btn btn-primary" href="{{url("efectosenfermedades/create")}}" title="Registrar efectos de las enfermedades">
                +</a>
            </div>
            @endif
        </div>
            ...
            <tr>
                ...
                <td>
                    ...
                    <table>
                        ...
                        @foreach($enfermedade->efectos as $efecto)
                            <tr>
                                ...
                                @if(Auth::user()->tipo_usuario==1)
                                <td>
                                    <form action="{{url("efectosenfermedades", $efecto->id_enfermedad_efecto)}}" method="POST">
                                        @csrf
                                        @method("DELETE")
    ...
    <table class="table table-bordered table-responsive-lg">
        ...
        @foreach ($efectosenfermedades as $efectoenfermedad)
            <tr>

                <td>
                    @foreach($enfermedades as $enfermedad)
                        @if($efectoenfermedad->id_enfermedad==$enfermedad->id_enfermedad)
                            {{$enfermedad->nombre}}
                            <br>
                ...
                <td>
                    @foreach($efectos as $efecto)
                        @if($efectoenfermedad->id_efecto_enfermedad==$efecto->id_efecto_enfermedad)
                            {{$efecto->efecto}}
                        @endif
                    @endforeach
                </td>
                ...
                @if(Auth::user()->tipo_usuario==1)
                <td>
                    <form action="{{url("efectosenfermedades", $efectoenfermedad->id_enfermedad_efecto)}}" method="POST">
                        @csrf
                        @method("DELETE")
                        <button id="btneliminar" class="btn btn-danger"><b>X</b></button>
                    </form>
